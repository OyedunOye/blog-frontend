import { articleItems, articleMenu, horizontalArticleCards, mainLatestArticle } from '@/constants'
        ...
        <MaxWidth className='my-6'>
                <div className="flex flex-col w-full ">
                    <nav className='flex h-10 content-center gap-3 my-3'>
                        ...
                        {articleMenu.map((menu) => (
                            <li key={menu} className="flex"><Button variant='ghost'>{menu}</Button></li>
                        ))}
                        </ul>
                    </nav>
                            ...
                            <Image src={lightBulbsImg} alt='light image' className='rounded-t-md' />
                                {mainLatestArticle.map((story) => (
                                    <div key={story.title} className="flex flex-col m-4 gap-2">
                                        ...
                                        <h3 className="font-semibold text-lg">UI Interactions of the week #261</h3>
                ...
                <div className="flex gap-4 my-4">
                    {horizontalArticleCards.map((card)=> (
                        <div key={card.title} className="flex flex-col w-72 gap-2 border rounded-md ">
                            <div className="flex ">
                                <Image src={card.articleImg} alt={card.title} width={288} height={250} className='rounded-t-md'/>

                            </div>
                            <div className="flex gap-2 text-sm text-gray-500 p-2">
                                <Image src={card.authorImg} alt={card.author} width={24} height={24} />
